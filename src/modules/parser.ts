...
import { CallExpression, Node, Program, Token } from '../types.ts';
...
export const parser = (tokens: Token[]): Program => {
  ...
  const walk = (): Node => {
    ...
    if (token.type === TOKEN.PARENTHESES && token.value === '(') {
      consumeToken();
      ...
      while (
        token.type !== TOKEN.PARENTHESES ||
        (token.type === TOKEN.PARENTHESES && token.value !== ')')
      ) {
